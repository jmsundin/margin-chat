...
const controls = { offline: false, delayMs: 0, authDelayMs: 0, signedIn: true, free: false, userId: "preview-user" };
...
const handler = createApiHandler({
  authService, database, documentService, vaultService,
  ...
  try {
    ...
    if (url.pathname === "/api/fixture") {
      if (request.method === "POST") {
        ...
        if (body.action === "remote") {
          ...
          for (const field of ["offline", "signedIn", "free"]) if (typeof body[field] === "boolean") controls[field] = body[field];
      ...
      return sendJson(response, 200, { controls, user: user(), manifest, activeApiRequests, projection: projection.get(controls.userId) ?? null, directory }, { "Cache-Control": "no-store" });
    }
    if (url.pathname.startsWith("/api/")) {
      ...
      try {
        ...
        if (url.pathname.startsWith("/api/chat")) return sendJson(response, 503, { error: "Model calls are intentionally unavailable in this isolated preview." });
