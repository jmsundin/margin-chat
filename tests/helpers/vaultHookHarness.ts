...
async function checkEmptyWorkspaceSettings() {
  ...
  await act(async () => {
    current.setState((state: any) => ({
      ...state, defaultServiceId: "openai-api", defaultModelId: "gpt-5.6",
      ...
      conversations: { ...state.conversations, [state.rootId]: {
        ...state.conversations[state.rootId], serviceId: "openai-api", modelId: "gpt-5.6",
      } },
    }));
  });
  ...
  const assertSelection = () => {
    ...
    assert.equal(current.state.defaultModelId, "gpt-5.6");
    assert.equal(current.state.conversations[current.state.rootId].serviceId, "openai-api");
    assert.equal(current.state.conversations[current.state.rootId].modelId, "gpt-5.6");
  ...
  const assertNoDocuments = async () => {
    ...
    assert.equal(settings.workspace.preferences.defaultModelId, "gpt-5.6");
  };
  assertSelection();
