...
const build = await Bun.build({
  entrypoints: [new URL("./chatHistoryPreview.tsx", import.meta.url).pathname],
  target: "browser",
  minify: false,
  sourcemap: "inline",
...
const html = `<!doctype html><html lang="en" data-theme="dark"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Chat history import preview</title>${styles.map((path) => `<link rel="stylesheet" href="${path}">`).join("")}<style>body{margin:0}button{cursor:pointer}</style></head><body><div id="root"></div>${scripts.map((path) => `<script type="module" src="${path}"></script>`).join("")}</body></html>`;
...
console.log(`Synthetic chat history preview: ${server.url}`);
