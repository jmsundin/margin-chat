...
function configuredProxy(target: string, port = "8787") {
  const previous = { BACKEND_URL: process.env.BACKEND_URL, BACKEND_PORT: process.env.BACKEND_PORT, PORT: process.env.PORT };
  try {
...
async function withProxy(handler: RequestListener, check: (origin: string, upstream: Server) => Promise<void>) {
  ...
  const proxy = await createViteServer({
    ...
    server: { middlewareMode: true, hmr: false, proxy: configuredProxy(`http://127.0.0.1:${upstreamAddress.port}`) },
  });
  ...
  try {
    ...
    await check(`http://127.0.0.1:${address.port}`, upstream);
  } finally {
